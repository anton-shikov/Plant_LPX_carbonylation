...
setwd("C:/Users/HP/Desktop/PPB/����������� ������� � ������ � ������������� �������/data")
...
(dif_dum.T <- c('a','a','a','a','c','a','b','b','b','b','c','d','d','d','d','c','b','b','b','b'))
...
for (i in 1:nrow(ag_df_percent)){
  sub1=anno_df_percent[which(anno_df_percent[,12]==as.vector(ag_df_percent[i,8] )),]
...
df2p <- data_summary(merged_df_percent, varname="measurement", 
                    groupnames=c("factor", "condition"))
...
pp <- ggplot(df2p, aes(fill=condition, y=measurement, x=factor)) + 
  geom_bar(position="dodge", stat = "summary", fun.y = "mean", color="black")+
  geom_errorbar(aes(ymin=measurement-sd, ymax=measurement+sd), width=.2,
                position=position_dodge(.9)) + theme_bw()+ 
  scale_y_continuous(limits=c(100,200),expand = expand_scale(mult = c(0, .1)),oob = rescale_none)
...
p + scale_x_discrete(labels=c("Рис корень", "Рис побег", "Пшеница корень",'Пшеница побег'))+
  ...
  theme(axis.title.x=element_blank(),
        ...
        panel.grid.minor =  element_blank())+ 
  geom_text(data =ag_df, 
            aes(x=factor, y=y_max+0.035,label=letters), 
            size = 10,position=position_dodge(0.9))




pp  + scale_x_discrete(labels=c("Рис корень", "Рис побег", "Пшеница корень",'Пшеница побег'))+scale_fill_manual(values = c("grey80", 'grey50','grey27','grey0'),labels=c(" Метилвиологен"," Менадион"," Перекись водорода"," Аскорбат +Cu(II)"))+
  ...
  geom_text(data =ag_df_percent, 
            ...
            size = 10,position=position_dodge(0.9))+geom_hline(yintercept=100, linetype="dashed", color = "black")
...
(dif_dum.T <- multcompLetters(dif_dum))
(dif_dum.T <- c('a','b','a','b','b','a','b','a','b','b','e','f','f','f','f','c','d','d','d','d'))
...
df3 <- data_summary(lmerged_df, varname="measurement", 
                    groupnames=c("factor", "condition"))
df3p <- data_summary(lmerged_df_percent, varname="measurement", 
                    groupnames=c("factor", "condition"))
...
lpp <- ggplot(df3p, aes(fill=condition, y=measurement, x=factor)) + 
  geom_bar(position="dodge", stat = "summary", fun.y = "mean", color="black")+
  geom_errorbar(aes(ymin=measurement-sd, ymax=measurement+sd), width=.2,
                position=position_dodge(.9)) + theme_bw()+ 
  scale_y_continuous(limits=c(95,185),expand = expand_scale(mult = c(0, .1)),oob = rescale_none)
...
lp +
  ...
  ylab(bquote('TBARS content, nmol/g fresh weight'))+
  xlab(NULL)+
  theme(axis.title.x=element_blank(),
        axis.ticks.x=element_blank(),
        axis.text.x=element_text(color='black',size=20),
...
lpp + scale_x_discrete(labels=c("Рис корень", "Рис побег", "Пшеница корень",'Пшеница побег'))+
  ...
  geom_text(data =lag_df_percent, 
            ...
            size = 10,position=position_dodge(0.9))+geom_hline(yintercept=100, linetype="dashed", color = "black",size =1)
...
lp + scale_x_discrete(labels=c("Рис корень", "Рис побег", "Пшеница корень",'Пшеница побег'))+
  ...
  theme(axis.title.x=element_blank(),
        ...
        panel.grid.minor =  element_blank())+ 
  geom_text(data =lag_df, 
            aes(x=factor, y=y_max+9,label=letters), 
            size = 10,position=position_dodge(0.9))
dev.off()
...
dif_dum.T$Letters <- c('ab','a','a','ab','ab','a',
                       'b','ab','bc','c','c','b',
                       'bc','bc','b','c','cd','c',                      
                       'bc','de','cd','e','e','e')
...
rag_df$letters <- dif_dum.T$Letters
...
df4p <- data_summary(rmerged_df_percent, varname="measurement", 
                    groupnames=c("factor", "condition"))
...
rpp <- ggplot(df4p, aes(fill=condition, y=measurement, x=factor)) + 
  geom_bar(position="dodge", stat = "summary", fun.y = "mean", color="black")+
  geom_errorbar(aes(ymin=measurement-sd, ymax=measurement+sd), width=.2,
                position=position_dodge(.9)) + theme_bw()+ 
  scale_y_continuous(limits=c(75,240),expand = expand_scale(mult = c(0, .1)),oob = rescale_none)
...
rp +
  ...
  theme(axis.title.x=element_blank(),
        ...
        panel.grid.major = element_blank(), 
        panel.grid.minor =  element_blank())+ 
  geom_text(legend.position = c(0.17, 0.89),
        data=rag_df, 
            aes(x=factor, y=y_max+0.03,label=letters), 
            size = 5,position=position_dodge(0.9))
dev.off()

rag_df_percent[9,]$y_max <- rag_df_percent[9,]$y_max+10

rpp + scale_x_discrete(labels=c("Рис корень", "Рис побег", "Пшеница корень",'Пшеница побег'))+
...
(dif_dum.T <- multcompLetters(dif_dum,threshold = 0.05))
(dif_dum.T <- c('c','d','d','d','e','a','a','b','a','a','a','c','h','c','e','e','i','g','f','f','f','f','f','g'))
#dif_dum.T$Letters <- c('a','ab','ab','a','a','a','a',
#                       'a','ab','b','ab','ab','ab','b',
#                       ,
                      # )
...
lrag_df$letters <-  dif_dum.T
lrag_df_percent$letters <- 'letter'
lrag_df_percent$letters <- dif_dum.T
...
df4p <- data_summary(lrmerged_df_percent, varname="measurement", 
                    groupnames=c("factor", "condition"))
...
lrpp <- ggplot(df4p, aes(fill=condition, y=measurement, x=factor)) + 
  geom_bar(position="dodge", stat = "summary", fun.y = "mean", color="black")+
  geom_errorbar(aes(ymin=measurement-sd, ymax=measurement+sd), width=.2,
                position=position_dodge(.9)) + theme_bw()+ 
  scale_y_continuous(limits=c(65,150),expand = expand_scale(mult = c(0, .1)),oob = rescale_none)
...
lrpp + scale_x_discrete(labels=c("Рис корень", "Рис побег", "Пшеница корень",'Пшеница побег'))+
  scale_fill_manual(values = c(  "grey93", 'grey75','grey57','grey33', 'grey0'),labels=c(" Аноксия"," 1ч"," 3ч"," 6ч"," 24ч"))+
  ylab(bquote('Содержание ТБК-реактивных продуктов, % от контроля'))+
  xlab(NULL)+
  theme(axis.title.x=element_blank(),
