...
setwd("C:/Users/HP/Desktop/PPB/����������� ������� � ������ � ������������� �������/data/WB")
...
ag_df$letters <- dif_dum.T$Letters 
...
p + scale_x_discrete(labels=c("Рис корень", "Рис побег", "Пшеница корень",'Пшеница побег'))+scale_fill_manual(values = c("white",'grey90' ,"grey76", 'grey60','grey47','grey20','grey5'),labels=c(" Контроль"," Аноксия", " Реаэрация"," Метилвиологен"," Менадион"," Перекись водорода"," Аскорбат +Cu(II)"))+
  ylab(bquote('Уровень карбонилирования белков, отн. ед.'))+
  xlab(NULL)+
...
p +scale_fill_manual(values = c("white",'grey90' ,"grey76", 'grey60','grey47','grey20','grey5'),labels=c(" Control"," Anoxia"," Re-aeration"," Methyl viologen"," Menadion"," Hydrogen peroxide"," Ascorbate +Cu(II)"))+
  ylab(bquote('Protein carbonylation level, relative units'))+
  xlab(NULL)+
  theme(axis.title.x=element_blank(),
        axis.ticks.x=element_blank(),
        axis.text.x=element_text(color='black',size=20),
